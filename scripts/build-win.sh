#!/usr/bin/env bash
#
# build-win.sh — 打包 MarkPad Windows x64 安装版 + 便携版
#
# 用法：bash scripts/build-win.sh
# 产物：release/MarkPad-<version>-x64-setup.exe / release/MarkPad-<version>-x64-portable.exe
...
# electron-builder 输出格式：MarkPad Setup x.y.z.exe / MarkPad x.y.z.exe
# 统一重命名为 MarkPad-<version>-x64-setup.exe / MarkPad-<version>-x64-portable.exe
